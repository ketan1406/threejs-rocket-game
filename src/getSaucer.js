...
import { Tween, Easing } from '@tweenjs/tween.js'; // Correct Import
import { calculateLookAtRotation } from "./utils.js"; // Corrected Typo
...
function getBolt(saucer) {
  ...
  bolt.boundingRadius = size * Math.sqrt(3); // Approximate for Icosahedron
  ...
  function fire(direction) {
    ...
    bolt.position.copy(saucer.position);
    console.log(`Saucer fired bolt in direction ${direction}`);
  }
...
function getSaucer(glb) {
  ...
  // Define boundingRadius based on geometry
  saucerGroup.boundingRadius = radius;
  ...
  function fire() {
    ...
    console.log(`Saucer fired bolt at angle ${angle}`);
  }
  let nextTime = 3000;
  function update(t) {
    if (!isTweening) {
  ...
  function _playHitAnimation() {
    if (!isTweening) {
      ...
      let alphaTween = new Tween(deadSaucer.userData.material)
        .to({ opacity: -1 }, 2000)
  ...
  function sense(targetPos) {
    targetDirection = calculateLookAtRotation(targetPos, saucerGroup);
  }
